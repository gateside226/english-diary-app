    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'user',
            content: `以下の英語日記を読んで、内容や状況を最もよく表す単語を日本語で最大3つ選び、タグとして返してください。

条件:
- 各タグは「#」で始める1〜4文字程度の名詞（例: #旅行, #仕事, #買い物, #友人, #天気, #食事, #勉強, #家族, #運動 など）
- 固定のカテゴリーはありません。日記の内容に最も合う言葉を自由に選んでください
- 似た状況の日記では同じ単語を使うようにし、表記のブレ（例: #旅行 と #トラベル）は避けてください
- JSON形式のみで返してください

日記:
${text}

レスポンス例: {"tags": ["#旅行", "#友人", "#食事"]}`
          }
        ]
      })
    });
